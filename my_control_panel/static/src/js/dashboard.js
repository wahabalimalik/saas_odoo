odoo.define('my_control_panel', function (require) {
"use strict";
...
var Dashboards = Widget.extend({
    ...
    load: function(dashboards){
        ...
        session.rpc("/my_control_panel/data", {}).then(function (data) {
            // Load each dashboard
    ...
    load_apps: function(data){
        return  new DashboardsApps(this, data.apps).replace(this.$('.o_my_control_panel_apps'));
    },

    load_share: function(data){
        return new DashboardsShare(this, data.share).replace(this.$('.o_my_control_panel_share'));
    },

    load_invitations: function(data){
        return new DashboardsInvitations(this, data.users_info).replace(this.$('.o_my_control_panel_invitations'));
    },

    load_planner: function(data){
        return  new DashboardsPlanner(this, data.planner).replace(this.$('.o_my_control_panel_planner'));
    },
});

var DashboardsInvitations = Widget.extend({
    template: 'DashboardsInvitations',
    events: {
        'click .o_my_control_panel_invitations': 'send_invitations',
        'click .o_my_control_panel_access_rights': 'on_access_rights_clicked',
        'click .o_my_control_panel_user': 'on_user_clicked',
        'click .o_my_control_panel_more': 'on_more',
    },
    init: function(parent, data){
        this.data = data;
        this.parent = parent;
    ...
    send_invitations: function(e){
        ...
        if (is_valid_emails) {
            ...
            new Model("res.users")
                ...
                .always(function() {
                    // Re-enable button
                    $(e.delegateTarget).find('.o_my_control_panel_invitations').prop('disabled', false);
...
var DashboardsPlanner = Widget.extend({

    template: 'DashboardsPlanner',

    events: {
        'click .o_my_control_panel_progress_title,.progress': 'on_planner_clicked',
    },
    ...
    update_planner_progress: function(){
        ...
        this.$('.o_my_control_panel_planners_list').replaceWith(
            QWeb.render("DashboardsPlanner.PlannersList", {'planners': this.planners})
        );
    },
    ...
    set_overall_progress: function(){
        ...
        self.$('.o_my_control_panel_planner_overall_progress').text(self.overall_progress);
    },
...
var DashboardsShare = Widget.extend({
    template: 'DashboardsShare',
    ...
    init: function(parent, data){
        ...
        this.share_url = '/web/underdevelop';
        this.share_text = encodeURIComponent("I am using #Backoffice24 - Awesome  business apps.");
    },
    ...
    share_twitter: function(){
        ...
        var popup_url = _.str.sprintf('/web/underdevelop', encodeURIComponent(this.share_url));
...
core.action_registry.add('my_control_panel.main', Dashboards);

return {
    Dashboards: Dashboards,
    DashboardsInvitations: DashboardsInvitations,
    DashboardsPlanner: DashboardsPlanner,
    DashboardsShare: DashboardsShare,
};
